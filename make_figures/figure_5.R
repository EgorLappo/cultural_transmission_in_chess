...
make_fitness_curve_plot <- function(data, title) {
  fitness_plot_grid <- ggplot(data, aes(xmin=lower_bp, xmax=upper_bp)) +
    ...
    theme_bw() + theme(axis.text.x = element_text(angle = 45), plot.title = element_text(face="bold"))
...
ggsave("../figures/figure_5a.pdf", qp_f,w=3.3,h=8.5,unit="in")

ck_f <- make_fitness_curve_plot(ck_fitness_data, "Caro-Kann, ply 5")
ggsave("../figures/figure_5b.pdf",ck_f,w=3.3,h=8.5,unit="in")

naj_f <- make_fitness_curve_plot(naj_fitness_data,"Najdorf Sicilian, ply 11") 
ggsave("../figures/figure_5c.pdf",naj_f,w=3.3,h=8.5,unit="in")
