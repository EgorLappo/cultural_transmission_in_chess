...
qp_beta_data = pd.read_csv("../model/model_results/queens_pawn_ply_2/mcmc_intervals_beta.csv")
ck_beta_data= pd.read_csv("../model/model_results/carokann_ply_5/mcmc_intervals_beta.csv")
naj_beta_data= pd.read_csv("../model/model_results/sicilian_najdorf_ply_11/mcmc_intervals_beta.csv")
...
qp_beta_data['variable'] = ['$\\beta_{win}$']*7 + ['$\\beta_{top50-win}$']*7 + ['$\\beta_{top50-freq}$']*7
ck_beta_data['variable'] = ['$\\beta_{win}$']*6 + ['$\\beta_{top50-win}$']*6 + ['$\\beta_{top50-freq}$']*6
naj_beta_data['variable'] = ['$\\beta_{win}$']*10 + ['$\\beta_{top50-win}$']*10 + ['$\\beta_{top50-freq}$']*10
...
variables = ['$\\beta_{top50-freq}$', '$\\beta_{top50-win}$', '$\\beta_{win}$',]
...
for x, sn in enumerate(strategies):
    ...
    axs[nrows[x]-1, x].set_xlabel('Value', fontsize=12)
...
fig.savefig('../figures/figure_6.png', dpi=500)