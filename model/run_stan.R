...
run_stan_model <- function(strategy_name) {
    ...
    win_data <- read.csv(paste0("data/",
                    strategy_name, "/win_rates.csv"))
    top_player_count_data <- read.csv(paste0("data/",
                    strategy_name, "/top_player_response_counts.csv"))
    top_player_win_data <- read.csv(paste0("data/",
    ...
    write.csv(freq_quantiles_by_strategy,
...
summarize_fits <- function(strategy_name) {
  ...
  responses <- paste0("data/",strategy_name,"/response_counts.csv") %>% read.csv() %>% colnames %>% tail(-1)
  n <- length(responses)

  fit <- readRDS(paste0("model_fits/",strategy_name,"/fit.RDS"))

  fit %>% as_draws_df() %>% write.csv(paste0("model_fits/",strategy_name,"/fit.csv"))
